  ...
  bool * taken)
{
  ...
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(
    ...
    return RMW_RET_INCORRECT_RMW_IMPLEMENTATION);
  RMW_CHECK_ARGUMENT_FOR_NULL(request_header, RMW_RET_INVALID_ARGUMENT);
  RMW_CHECK_ARGUMENT_FOR_NULL(serialized_response, RMW_RET_INVALID_ARGUMENT);
  RMW_CHECK_ARGUMENT_FOR_NULL(taken, RMW_RET_INVALID_ARGUMENT);
  ...
  *taken = false;
  ...
  response.buffer_.reset(new(std::nothrow) eprosima::fastcdr::FastBuffer());
  ...
  if (ReturnCode_t::RETCODE_OK == info->response_reader_->take(data_values, info_seq, 1)) {
    if (info_seq[0].valid_data) {
        ...
        response.sample_identity_.writer_guid() == info->writer_guid_)
      {
        ...
        auto buffer_size = static_cast<size_t>(response.buffer_.get()->getBufferSize());
        if (serialized_response->buffer_capacity < buffer_size) {
          auto ret = rmw_serialized_message_resize(serialized_response, buffer_size);
          if (ret != RMW_RET_OK) {
            return ret;  // Error message already set
          }
        }
        serialized_response->buffer_length = buffer_size;
        memcpy(
          serialized_response->buffer,
          response.buffer_.get()->getBuffer(),
          serialized_response->buffer_length);
  ...
  const rmw_serialized_message_t * serialized_response)
{
  ...
  eprosima::fastcdr::FastBuffer buffer(
    reinterpret_cast<char *>(serialized_response->buffer), serialized_response->buffer_length);
  ...
  if (!ser.jump(serialized_response->buffer_length)) {
